...
class MockServer(socketserver.ThreadingMixIn, socketserver.TCPServer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(t, DockerThread):
            self.ip = ipaddress.IPv4Address(t.get_ip(t.container.name))
        else:
            self.ip = None
    ...
    def await_packet(self, packet_type, timeout):
        try:
            ...
            if isinstance(packet, tuple):
                ...
                ValueError(f'MockServer received unexpected packet type. Expected {packet_type} Got: {type(packet)}')
        except queue.Empty:
            return None
...
class MockClient:
    def __init__(self, req: Packet, ip=None, port=1400):
        ...
        self.resolvedName = threading.Event()
    ...
    def run(self):
        ...
        try:
            if not self.ip:
                self.ip = threading.current_thread().get_ip('hash-peer')
            self.resolvedName.set()
...
class GeneralPktHandler(socketserver.StreamRequestHandler):
    ...
    def handle_data_packet(self, data):
        ...
        if len(data) < 7:
            raise ValueError('Peer did not send enough bytes to parse a data/client packet header')
    ...
    def get_first_byte(self):
        ...
        if self.rfile not in readable:
            raise ValueError("Peer did not send a single byte to determine packet type!")
...
class ControlPktHandler(GeneralPktHandler):
    def handle(self):
        ...
        if not Packet.packet_type(data) == ControlPacket:
            raise ValueError('Expected control packet but control bit is not set!')
...
class DataPktHandler(GeneralPktHandler):
    def handle(self):
        ...
        if not Packet.packet_type(data) == DataPacket:
            raise ValueError('Expected data/client packet but control bit is set!')
    ...
    def send_response(self):
        try:
            if self.server.response_timeout is not None:
                packet = self.server.resp_q.get(timeout=self.server.response_timeout)
            else:
